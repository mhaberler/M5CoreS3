...
int8_t I2C_IMU::bmi2_i2c_write(uint8_t reg_addr, const uint8_t *reg_data,
...
void I2C_IMU::bmi2_delay_us(uint32_t period, void *intf_ptr) {
    delayMicroseconds(period);
}

int8_t I2C_IMU::aux_i2c_read(uint8_t reg_addr, uint8_t *reg_data,
                             uint32_t length, void *intf_ptr) {
    int8_t ret;
    ret = bmi2_read_aux_man_mode(reg_addr, reg_data, length, &aux_bmi2_dev);
    return ret;
}
int8_t I2C_IMU::aux_i2c_write(uint8_t reg_addr, const uint8_t *reg_data,
                              uint32_t length, void *intf_ptr) {
    int8_t ret;
...
float I2C_IMU::lsb_to_mps2(int16_t val, float g_range, uint8_t bit_width) {
    float half_scale = ((float)(1 << bit_width) / 2.0f);

    return (GRAVITY_EARTH * val * g_range) / half_scale;
}

float I2C_IMU::lsb_to_dps(int16_t val, float dps, uint8_t bit_width) {
    float half_scale = ((float)(1 << bit_width) / 2.0f);
